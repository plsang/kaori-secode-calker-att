function calker_late_fusion_segment(proj_name, test_pat, suffix)
	...
	calker_root_exp_dir = sprintf('%s/%s/experiments', ker.proj_dir, proj_name);
	
	fused_ids = fieldnames(ker_names);
	fusion_name = 'fusion-multiseg';
	for ii=1:length(fused_ids),
		fusion_name = sprintf('%s.%s', fusion_name, fused_ids{ii});
	end
	
	output_file = sprintf('%s/trecvidmed13-100000-calker/%s%s/scores/%s/%s.scores.mat', calker_root_exp_dir, fusion_name, suffix, test_pat, fusion_name);
	output_dir = fileparts(output_file);
	if ~exist(output_dir, 'file'),
		mkdir(output_dir);
	end
	
	segments = {'trecvidmed13-100000-calker', 'trecvidmed13-90-calker'};
	suffix_list = {'--calker-v7.1', '--calker-v7.2'};
	...
	for ii=1:length(events),
		...
		for jj = 1:length(segments),
			exp_name = segments{jj};
			ker_name = ker_names.(fused_ids{1});
			fprintf(' -- [%d/%d] kernel [%s - %s]...\n', jj, length(segments), exp_name, ker_name);
			scorePath = sprintf('%s/%s/%s%s/scores/%s/%s.video.scores.mat', calker_root_exp_dir, exp_name, ker_name, suffix_list{jj}, test_pat, ker_name);
			
			if ~exist(scorePath, 'file');
				warning('--- CHECK IF VIDEO BASED RUNNING ---\n');
				scorePath = sprintf('%s/%s/%s%s/scores/%s/%s.scores.mat', calker_root_exp_dir, exp_name, ker_name, suffix_list{jj}, test_pat, ker_name);
			end
	...
	calker_cal_map(proj_name, 'trecvidmed13-100000', ker, events);
end